...
class LoginFragment : Fragment() {
        ...
        auth.signInWithCredential(credential)
            .addOnCompleteListener { task ->
                ...
                if (task.isSuccessful) {
                    ...
                    mess.log("Google auth error: $errorMessage")
                    mess.toast(errorMessage)
                }
            }
            .addOnFailureListener { exception ->
                mess.pbDismiss()
                mess.log("Google auth failure: ${exception.message}")
                mess.toast("Authentication failed: ${exception.message}")
            }
    }
                    ...
                    auth.signInWithEmailAndPassword(email, password)
                        .addOnCompleteListener() { task ->
                            ...
                            if (task.isSuccessful) {
                                ...
                                mess.log("Login error: $errorMessage")
                                mess.toast(errorMessage)
                            }
                        }
                        .addOnFailureListener { exception ->
                            mess.pbDismiss()
                            mess.log("Login failure: ${exception.message}")
                            mess.toast("Login failed: ${exception.message}")
                        }
                } else {
                    mess.toast("Please enter a valid email address")
                    mess.pbDismiss()
    ...
    private fun navigate() {
        viewModel.isPresent { isPresent ->
            if (isPresent) {
                mess.log("User is present in Firestore")
                fireBase.getUser(auth.currentUser?.uid.toString(),
                    ...
                    onFailure = {
                        mess.log("Error fetching user data: ${it.message}")
                        mess.pbDismiss()
                        // User exists in Firestore but failed to load - redirect to complete profile
                        mess.toast("Error loading profile. Please complete your profile.")
                        findNavController().navigate(R.id.action_loginFragment_to_detailsFragment)
                    })
            } else {
                mess.log("User not present in Firestore - redirecting to complete profile")
                mess.pbDismiss()
