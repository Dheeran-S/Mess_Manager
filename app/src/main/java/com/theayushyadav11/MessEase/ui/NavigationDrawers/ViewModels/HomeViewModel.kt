...
class HomeViewModel(val menuDao: MenuDao) : ViewModel() {
    ...
    fun getDayPolls(user: User, date: String, onSuccess: (List<Poll>) -> Unit) {
        try {
            // Skip if user data is empty
            if (user.batch.isEmpty() || user.passingYear.isEmpty() || user.gender.isEmpty()) {
                Log.d(TAG, "User data incomplete, skipping polls")
                onSuccess(listOf())
                return
            }
            ...
            firestoreReference.collection(POLLS).whereEqualTo(DATE, date)
                .addSnapshotListener { value, error ->
                    if (error != null) {
                        Log.d(TAG, "Error fetching polls: ${error.message}")
                        onSuccess(listOf())
                    ...
                    try {
                        ...
                        Log.e(TAG, "Error processing polls: ${e.message}")
                        e.printStackTrace()
                        onSuccess(listOf())
                    }
                }
        } catch (e: Exception) {
            Log.e(TAG, "Error in getDayPolls: ${e.message}")
            e.printStackTrace()
            onSuccess(listOf())
        }
    }
    ...
    fun getDayParticulars(context: Context, day: Int, onSuccess: (List<Particulars>) -> Unit) {
        try {
            Mess(context).getMainMenu {
                try {
                    if (it.menu.isEmpty()) {
                        Log.d(TAG, "Menu list is empty")
                        onSuccess(listOf())
                        return@getMainMenu
                    }
                    if (day < 0 || day >= it.menu.size) {
                        Log.d(TAG, "Invalid day index: $day, menu size: ${it.menu.size}")
                        onSuccess(listOf())
                        return@getMainMenu
                    }
                    ...
                    Log.e(TAG, "Error getting day particulars: ${e.message}")
                    e.printStackTrace()
                    onSuccess(listOf())
                }
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error in getDayParticulars: ${e.message}")
            e.printStackTrace()
            onSuccess(listOf())
        }
    }
    ...
    fun getDayMsgs(user: User, date: String, onResult: (List<Msg>) -> Unit) {
        try {
            // Skip if user data is empty
            if (user.batch.isEmpty() || user.passingYear.isEmpty() || user.gender.isEmpty()) {
                Log.d(TAG, "User data incomplete, skipping messages")
                onResult(listOf())
                return
            }
            ...
            firestoreReference.collection(MESSAGES).orderBy(COMPARER, DESCENDING_ORDER)
                .addSnapshotListener { value, error ->
                    if (error != null) {
                        Log.d(TAG, "Error fetching messages: ${error.message}")
                        onResult(listOf())
                        return@addSnapshotListener
                    }
                    try {
            ...
            Log.e(TAG, "Error in getDayMsgs: ${e.message}")
            e.printStackTrace()
            onResult(listOf())
        }
    }
